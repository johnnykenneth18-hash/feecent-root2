...
async function validateAndPriceBillRequest({
  ...
  return {
    ...
    pricing: {
      ...
      gateway_code: provider.gateway_code,
      external_biller_code: provider.external_biller_code,
      // FIX: this was hardcoded null, which is why every ELECTRICITY
      // purchase through Flutterwave has been failing at the provider
      // call — flutterwave-service.js's payElectricity() requires
      // itemCode (the prepaid/postpaid variant selector) and rejects
      // the request outright when it's missing. bills-worker.js reads
      // itemCode from bill_transactions.external_plan_code (same
      // column plan-based categories use for their plan's item code —
      // see bills-worker.js's own comment on that field doing double
      // duty), so variable-amount categories that need one (currently
      // just ELECTRICITY, for both Flutterwave and VTpass — VTpass's
      // payElectricity needs the identical prepaid/postpaid selector,
      // same column, same worker code path) now source it from
      // bill_providers.external_item_code — admin-set per provider
      // row (Providers tab -> External Item Code), same manual-entry
      // pattern external_biller_code already uses. null for anything
      // that doesn't need one (AIRTIME, BETTING) — harmless, same as
      // before.
      external_plan_code: provider.external_item_code || null,
    },
  };
}
