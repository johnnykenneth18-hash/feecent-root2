...
class PaymentProvider {
  ...
  // Funds an external wallet (currently: Frozyla) from a FEECENT
  // user's balance. { customerIdentifier, amount, reference } ->
  // { success, retryable, error, data, raw } — same shape every other
  // purchase method here returns. Not payment-processor-specific the
  // way purchaseAirtime/payElectricity etc. are (those exist because
  // Flutterwave/VTpass define those exact product categories) — this
  // exists because bills-worker.js's CATEGORY_GATEWAY_METHOD needs
  // some method name for the FROZYLA category, and declaring it here
  // keeps every provider method discoverable in one place rather than
  // duck-typed only on frozyla-provider.js.
  async fundWallet(_params) {
    throw new NotImplementedError(this.code, "fundWallet");
  }
