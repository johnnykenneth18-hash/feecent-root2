...
const PROVIDER_IMPLEMENTATIONS = {
  ...
  frozyla: require("./frozyla-provider"),
...
class ServiceRegistry {
  ...
  async resolve(serviceCode) {
    const { data: service, error: serviceErr } = await supabase
      .from("services")
      .select(
        "manual_override_provider_id, providers!services_manual_override_provider_id_fkey(id, code, name, is_active)",
      )
      .eq("code", serviceCode)
      .maybeSingle();
    ...
    if (serviceErr) {
      throw new Error(
        `Service lookup failed for '${serviceCode}': ${serviceErr.message}`,
      );
    }
  ...
  async clearManualOverride(serviceCode) {
    const { error } = await supabase
      .from("services")
      .update({
        manual_override_provider_id: null,
