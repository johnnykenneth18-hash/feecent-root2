...
async function alertFinalizeFailure(
  transferId,
  rpcName,
  rpcErr,
  knownStatus,
 ...
 */
async function finalizeVerifiedTransfer({
  reference,
  verified,
  providerCode,
  webhookLogUpdate,
  ...
  if (verified.status === "SUCCESSFUL") {
    ...
    // fetch the transfer's user_id + amount + beneficiary for the push text
    /*const { data: fullTransfer } = await supabase
      .from("flutterwave_transfers")
      .select("user_id, amount, beneficiary_name")
      .eq("id", transfer.id)
      .single();*/

    const { data: fullTransfer } = await supabase
      .from("flutterwave_transfers")
      .select("user_id, amount, beneficiary_name, account_number, bank_code, bank_name")
      .eq("id", transfer.id)
      .single();
    ...
    if (fullTransfer) {
      const { data: notif } = await supabase
        .from("notifications")
        .select("id")
        .eq("user_id", fullTransfer.user_id)
        .eq("type", "success")
      ...
      await sendPushNotificationForInAppNotification(
        fullTransfer.user_id,
        "Transfer successful",
        `Your transfer of NGN ${fullTransfer.amount} to ${fullTransfer.beneficiary_name} was successful.`,
        notif?.id || null,
        "transfer",
      );
    }
    ...
    triggerVatForCompletedTransfer(transfer.id).catch((err) =>
      console.error(
        `[TRANSFER-FINALIZE] VAT trigger threw for transfer ${transfer.id}:`,
        err,
      ),
    );
    ...
    const { error: rpcErr } = await supabase.rpc("fail_external_transfer", {
      p_transfer_id: transfer.id,
      p_reason:
        verified.complete_message ||
        `${providerCode} reported transfer as failed`,
