...
async function creditDeposit({ account, verified, providerCode }) {
  ...
  if (!result.duplicate) {
    // Fetch the notification row process_deposit() just created, so we
    // can pass its real id through for the push payload's deep-link.
    ...
    await sendPushNotificationForInAppNotification(
      account.user_id,
      "Deposit received",
      `You have received ${verified.currency || "NGN"} ${verified.amount} into your Feecent Wallet.`,
      notif?.id || null,
      "deposit",
    );
    
    if (verified.sender_name && verified.sender_account) {
      accountResolutionCache
        .cacheDepositSender({
          senderName: verified.sender_name,
          senderAccount: verified.sender_account,
          senderBank: verified.sender_bank,
          senderBankCode: null, // Flutterwave doesn't always provide this
        })
        .catch((err) => console.error("[DEPOSIT] Cache sender failed:", err));
    }
  }
  ...
  retryPendingVatForAccount(account.id).catch((err) =>
    console.error(
      `[DEPOSIT-CREDIT] VAT retry threw for account ${account.id}:`,
      err,
    ),
  );
