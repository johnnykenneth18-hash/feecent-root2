...
async function auditLog(adminId, actionType, details) {
  try {
    ...
    console.error(
      `[BILLS-ADMIN] audit log write failed for ${actionType}:`,
      err.message,
    );
  }
}
...
async function createCategory(adminId, req, fields) {
  ...
  await auditLog(adminId, "bills_admin_category_create", {
    category_id: data.id,
    fields,
  });
  safeLogAdminAction(req, "bills_admin_category_create", {
    category_id: data.id,
  });
  return data;
}
...
async function updateCategory(adminId, req, categoryId, fields) {
  ...
  await auditLog(adminId, "bills_admin_category_update", {
    category_id: categoryId,
    fields,
  });
  safeLogAdminAction(req, "bills_admin_category_update", {
    category_id: categoryId,
    fields,
  });
  return data;
...
async function deleteCategory(adminId, req, categoryId) {
  ...
  if (error) throw error;
  await catalog.invalidateCatalogCache({ categoryId });
  await auditLog(adminId, "bills_admin_category_delete", {
    category_id: categoryId,
  });
  safeLogAdminAction(req, "bills_admin_category_delete", {
    category_id: categoryId,
  });
  return data;
}
...
async function createProvider(adminId, req, fields) {
  ...
  await auditLog(adminId, "bills_admin_provider_create", {
    provider_id: data.id,
    fields,
  });
  safeLogAdminAction(req, "bills_admin_provider_create", {
    provider_id: data.id,
  });
  return data;
}
...
async function updateProvider(adminId, req, providerId, fields) {
  ...
  await auditLog(adminId, "bills_admin_provider_update", {
    provider_id: providerId,
    fields,
  });
  safeLogAdminAction(req, "bills_admin_provider_update", {
    provider_id: providerId,
    fields,
  });
  return data;
...
async function deleteProvider(adminId, req, providerId) {
  ...
  await auditLog(adminId, "bills_admin_provider_delete", {
    provider_id: providerId,
  });
  safeLogAdminAction(req, "bills_admin_provider_delete", {
    provider_id: providerId,
  });
  return data;
}
...
async function createPlan(adminId, req, fields) {
  ...
  await auditLog(adminId, "bills_admin_plan_create", {
    plan_id: data.id,
    fields,
  });
...
async function updatePlan(adminId, req, planId, fields) {
  ...
  await auditLog(adminId, "bills_admin_plan_update", {
    plan_id: planId,
    fields,
  });
  safeLogAdminAction(req, "bills_admin_plan_update", {
    plan_id: planId,
    fields,
  });
  return data;
...
async function refreshPlansFromProvider(adminId, req, providerId) {
  ...
  if (!result.success) {
    const err = new Error(
      result.error || "Failed to fetch plans from provider",
    );
    err.code = "PROVIDER_FETCH_FAILED";
    throw err;
  }

  const existingPlans = await catalog.getPlansForProvider(providerId, {
    includeHidden: true,
  });
  const existingByCode = new Map(
    existingPlans.map((p) => [p.external_plan_code, p]),
  );
  ...
  await auditLog(adminId, "bills_admin_plans_refreshed", {
    provider_id: providerId,
    created,
    updated,
  });
  safeLogAdminAction(req, "bills_admin_plans_refreshed", {
    provider_id: providerId,
    created,
    updated,
  });
...
async function upsertPricingRule(
  adminId,
  req,
  { category_id, provider_id, pricing_mode, markup_percent, fixed_fee },
) {
  const { data, error } = await supabase
    .from("bill_pricing")
    .upsert(
  ...
  await auditLog(adminId, "bills_admin_pricing_update", {
    category_id,
    provider_id,
    pricing_mode,
    markup_percent,
  ...
  safeLogAdminAction(req, "bills_admin_pricing_update", {
    category_id,
    provider_id,
  });
  return data;
...
async function getCategoryAnalytics() {
  ...
  for (const row of todayRows || []) {
    ...
    if (row.status === "completed") {
      ...
      bucket.profit_today +=
        Number(row.amount) + Number(row.fee_amount) - Number(row.provider_cost);
  ...
  return Object.values(byCategory).map((b) => ({
    ...
    success_rate:
      b.purchases_today > 0
        ? Math.round((b.completed / b.purchases_today) * 1000) / 10
        : null,
    failure_rate:
      b.purchases_today > 0
        ? Math.round((b.failed / b.purchases_today) * 1000) / 10
        : null,
  }));
}
