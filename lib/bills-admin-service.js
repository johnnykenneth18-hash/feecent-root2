...
async function refreshPlansFromProvider(adminId, req, providerId) {
  ...
  for (const item of result.items) {
    ...
    if (existing) {
      ...
      if (item.external_item_code) fields.external_item_code = item.external_item_code;
      ...
      await supabase.from("bill_plans").insert({
        ...
        external_item_code: item.external_item_code || null,
...
// ------------------------------------------------------------
// Preview-only variant of the above for providers on categories with
// NO plan selection (ELECTRICITY today) — those never call
// refreshPlansFromProvider(), so there was previously no way to
// discover a biller's item_code other than manually digging through a
// raw API response or reading Flutterwave's docs. This calls the
// EXACT same listBillerItems() as refreshPlansFromProvider() (nothing
// new fetched, nothing invented) but writes nothing to the DB — it
// just returns the list so the admin panel can show a picker and let
// the admin choose one to save as bill_providers.external_item_code
// via the normal updateProvider() call. Also useful for plan-based
// providers if you want to eyeball available item_codes without
// touching bill_plans.
// ------------------------------------------------------------
async function fetchProviderItemCodes(providerId) {
  const provider = await catalog.getProviderById(providerId);
  if (!provider) {
    const err = new Error("Provider not found");
    err.code = "NOT_FOUND";
  ...
  if (!provider.external_biller_code) {
    const err = new Error(
      `${provider.name} has no External Biller Code set — add it first (Providers tab → Edit) before fetching item codes.`,
    );
    err.code = "MISSING_BILLER_CODE";
    throw err;
  }
  ...
  if (!result.success) {
    const err = new Error(result.error || "Failed to fetch items from provider");
    err.code = "PROVIDER_FETCH_FAILED";
    throw err;
  }

  return result.items
    .filter((item) => item.external_item_code) // nothing to pick if the provider didn't return one
    .map((item) => ({
...
// ------------------------------------------------------------
// Frozyla integration monitoring (spec section 21). Deliberately
// READ + ANNOTATE ONLY — nothing here ever writes bill_transactions.status
// or moves money. That's finalize_bill_transaction()'s job exclusively
// (called from bills-worker.js's poll loop or frozyla-webhook-handler.js),
// never from an admin action. resolveFrozylaMismatch() below only ever
// touches the reconciliation_status/notes columns — an admin can mark a
// flagged discrepancy as reviewed, never mark a transaction "successful"
// or adjust its amount.
// ------------------------------------------------------------

async function listFrozylaTransactions({
  status,
  ...
  let query = supabase
    .from("bill_transactions")
    .select(
      "id, user_id, customer_identifier, amount, fee_amount, status, reconciliation_status, provider_reference, provider_tx_ref, created_at, completed_at, bill_categories!inner(code)",
      { count: "exact" },
    )
  ...
  if (reconciliation_status) query = query.eq("reconciliation_status", reconciliation_status);
...
async function getFrozylaTransactionDetail(billTransactionId) {
  const { data: bill, error } = await supabase
    .from("bill_transactions")
    .select("*, bill_categories!inner(code)")
  ...
  const { data: webhookLogs, error: webhookErr } = await supabase
    ...
    .select("*")
    .eq("transaction_reference", billTransactionId)
    .order("received_at", { ascending: false });
  if (webhookErr) throw webhookErr;

  return { ...bill, webhook_logs: webhookLogs || [] };
}

// reason is required and stored verbatim — this is the audit trail for
// "why did a human decide this flagged mismatch didn't need further
// action," not a free pass to skip explaining.
async function resolveFrozylaMismatch(adminId, req, billTransactionId, { reason } = {}) {
  if (!reason || !reason.trim()) {
    const err = new Error("A reason is required to resolve a flagged reconciliation entry");
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      reconciliation_resolved_by: adminId,
  ...
  await auditLog(adminId, "frozyla_reconciliation_resolved", { bill_transaction_id: billTransactionId, reason });
  safeLogAdminAction(req, "frozyla_reconciliation_resolved", { bill_transaction_id: billTransactionId, reason });
  return data;
}

async function triggerFrozylaReconciliation(adminId, req) {
  const { runFrozylaReconciliationSweep } = require("./frozyla-reconciliation-service");
  const result = await runFrozylaReconciliationSweep();
  await auditLog(adminId, "frozyla_reconciliation_triggered", result);
  safeLogAdminAction(req, "frozyla_reconciliation_triggered", result);
  return result;
}
...
module.exports = {
  ...
  fetchProviderItemCodes,
  upsertPricingRule,
  getSetting,
  setSetting,
  getCategoryAnalytics,
  listFrozylaTransactions,
  getFrozylaTransactionDetail,
  resolveFrozylaMismatch,
  triggerFrozylaReconciliation,
};