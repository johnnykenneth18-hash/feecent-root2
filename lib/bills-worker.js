...
async function processBillTransactionJob(job) {
  ...
  // GUARD (added after a real incident): if PaymentGateway has no
  // method for this category — e.g. purchaseData was missing before
  // a payment-gateway.js fix — calling it threw a bare TypeError,
  // which fell into the generic catch below, retried with backoff,
  // and once retries exhausted just marked the JOB failed while
  // leaving the bill_transaction (and its reserved funds) stuck in
  // "reserved" forever — no provider was ever contacted, so this is
  // never ambiguous and is always safe to fail immediately.
  if (typeof PaymentGateway[gatewayMethod] !== "function") {
    await failPermanently(
      bill,
      job,
      `PaymentGateway has no method '${gatewayMethod}' wired up for category '${categoryCode}' — this is a code/config bug, not a provider failure. No request was ever sent to any provider, so releasing the reservation is safe.`,
    );
    return;
  }
