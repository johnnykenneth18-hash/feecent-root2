...
/**
 * Creates a permanent Dedicated Virtual Account. Requires the customer
 * to have passed Paystack's BVN validation for a permanent
 * (non-expiring) account on most preferred_bank options.
 *
 * BVN-ONLY IS NOT A GAP HERE — it's Paystack's actual API. Their
 * Validate Customer docs are explicit: "Bank account validation
 * requires that you provide the customer's BVN and a bank account
 * connected to that BVN." There is no NIN path anywhere in Paystack's
 * identity-verification API (unlike Flutterwave, which accepts either).
 * A NIN-only signup must not be routed to Paystack — see
 * virtual-account-worker.js's provider-selection override, which sends
 * NIN-only jobs to Flutterwave instead of calling this function at all.
 * `nin` is accepted as a parameter here only so callers don't need a
 * separate function signature, but it is never used or sent to
 * Paystack — if bvn is absent this always fails, on purpose.
 */
async function createVirtualAccount({
  ...
  if (!bvn || !/^\d{11}$/.test(bvn)) {
    return {
      success: false,
      error: nin
        ? "Paystack requires a BVN and does not support NIN-only verification — this signup needs to go through Flutterwave instead"
        : "Invalid or missing BVN",
      retryable: false, // not a transient failure — retrying with the same payload will never succeed
    };
  }
