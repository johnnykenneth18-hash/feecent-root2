...
// token) isn't known until bills-worker.js finishes processing,
// seconds later, asynchronously. The frontend polls this endpoint
// after a purchase to find out when that's done, rather than the
// backend trying to push it.
// ------------------------------------------------------------
async function handleGetBillStatus(req, res) {
  try {
    ...
    const { data: bill, error } = await supabase
      ...
      .select(
        "id, status, failure_reason, token, network, amount, fee_amount, customer_identifier, completed_at, bill_categories(code, name), bill_providers(code, name)",
      )
      .eq("id", id)
