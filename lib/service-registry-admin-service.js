...
// ------------------------------------------------------------
// Manual override — the "switch providers manually even if automatic
// fails" control. Set forces a specific provider for a service; clear
// reverts to priority/health-based automatic routing.
// ------------------------------------------------------------
async function setManualOverride(adminId, req, serviceCode, providerId, reason) {
  await ServiceRegistry.setManualOverride(serviceCode, providerId);
  await auditLog(adminId, "registry_manual_override_set", { service_code: serviceCode, provider_id: providerId, reason });
  safeLogAdminAction(req, "registry_manual_override_set", { service_code: serviceCode, provider_id: providerId });
  return { service_code: serviceCode, provider_id: providerId };
}

async function clearManualOverride(adminId, req, serviceCode) {
  await ServiceRegistry.clearManualOverride(serviceCode);
  await auditLog(adminId, "registry_manual_override_clear", { service_code: serviceCode });
  safeLogAdminAction(req, "registry_manual_override_clear", { service_code: serviceCode });
  return { service_code: serviceCode };
}

async function getOverrideStatus(serviceCode) {
  const { data, error } = await supabase
    .from("services")
    .select("manual_override_provider_id, manual_override_set_at, providers!services_manual_override_provider_id_fkey(code, name)")
    .eq("code", serviceCode)
    .maybeSingle();
  if (error) throw error;
  return data;
...
// Providers that DECLARE this capability, whether or not they're
// currently routed to it — backs the admin UI's "add provider to this
// service" and "force this provider" dropdowns, so only valid choices
// are ever offered (nothing the operator picks there can fail
// assertCapability()).
async function listEligibleProviders(serviceCode) {
  const { data, error } = await supabase
    .from("provider_capabilities")
    .select("providers(id, code, name, is_active, health_status)")
    .eq("service_code", serviceCode)
    .eq("is_active", true);
  if (error) throw error;
  return (data || []).map((row) => row.providers).filter(Boolean);
}
