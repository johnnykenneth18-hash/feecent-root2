...
//      retries on backoff.
//
// This worker only ever CALLS Flutterwave and records what it said.
// It never marks a transfer completed on its own — completion only
// happens once the outbound webhook (transfer-webhook-service.js) or
// this worker's own status re-check confirms Flutterwave actually
...
async function failOrRetryJob(job, errorMessage) {
  ...
  if (nextRetryCount >= job.max_retry) {
    ...
    // Exhausted retries without a confirmed outcome from Flutterwave —
    ...
    await supabase.from("reconciliation_alerts").insert({
      ...
      status: "open",
      severity: "critical",
      notes: `send_external_transfer job ${job.id} (transfer ${job.payload.transfer_id}) exhausted retries with unknown final Flutterwave status: ${errorMessage}. Do not assume success or failure — verify with Flutterwave dashboard before touching the reservation.`,
    });
...
async function processSendExternalTransferJob(job) {
  ...
  // If we already sent this to Flutterwave on a previous attempt
  // (flutterwave_reference is set) but never confirmed the outcome —
  // e.g. the process died right after initiateTransfer() succeeded —
  // check status instead of calling initiateTransfer() again. Firing
  // a second payout for the same transfer would send the money twice.
  if (transfer.flutterwave_reference) {
    const statusCheck = await flutterwaveService.getTransferStatus(
    ...
    await applyFlutterwaveStatus(transfer, statusCheck.data, job);
    return;
  }
  ...
  if (!result.success) {
    if (result.retryable) {
      ...
      // itself errors (as it just did, from an unrelated SQL bug), this
      // is otherwise the only place Flutterwave's actual rejection
      // reason ever appears. Losing it here means losing it entirely.
      console.error(
        `[EXT-WORKER] Flutterwave rejected transfer ${transfer_id}: ${result.error}`,
      ...
      const { error: rpcErr } = await supabase.rpc("fail_external_transfer", {
        ...
        p_failure_code: "REJECTED_BY_FLUTTERWAVE",
      });
      if (rpcErr) {
        console.error(
  ...
  // Flutterwave accepted the request but that's not proof of delivery —
...
async function applyFlutterwaveStatus(transfer, flwData, job) {
  if (flwData.status === "SUCCESSFUL") {
    const { error: rpcErr } = await supabase.rpc("complete_external_transfer", {
      p_transfer_id: transfer.id,
      p_flw_transaction_id: String(flwData.id),
      p_flw_status: flwData.status,
    });
    if (rpcErr) {
      console.error(
        "[EXT-WORKER] complete_external_transfer RPC failed:",
        rpcErr,
      );
      await failOrRetryJob(
        job,
        `Confirmed successful by Flutterwave but failed to record completion: ${rpcErr.message}`,
      );
      return;
    }
    await markJobCompleted(job.id);
    console.log(
      `[EXT-WORKER] Job ${job.id} completed — transfer ${transfer.id} SUCCESSFUL`,
    );
    return;
  }
  ...
  if (flwData.status === "FAILED") {
    const { error: rpcErr } = await supabase.rpc("fail_external_transfer", {
      p_transfer_id: transfer.id,
      p_reason:
        flwData.complete_message || "Flutterwave reported transfer as FAILED",
      p_failure_code: "FLW_FAILED",
    });
    if (rpcErr) {
      console.error("[EXT-WORKER] fail_external_transfer RPC failed:", rpcErr);
      await failOrRetryJob(
        job,
        `Confirmed failed by Flutterwave but failed to release reservation: ${rpcErr.message}`,
      );
      return;
    }
    await markJobCompleted(job.id);
  ...
  // NEW / PENDING — still in flight on Flutterwave's side. Requeue this
  // job to check again later rather than treating it as either outcome.
  await failOrRetryJob(
    job,
    `Flutterwave status still ${flwData.status}, will re-check`,
  );
}
