...
const PaymentGateway = {
  ...
  // --- purchaseData/payElectricity/payCable/payBetting below: FLAGGED,
  // NOT REMOVED. bills-worker.js no longer calls these (nor does
  // anything else, as of this rebuild) — it now calls
  // ServiceRegistry.getProviderByCode(bill.gateway_code) directly for
  // every bill purchase, because these auto-routed methods pick
  // "whichever provider currently wins routing priority for this
  // category generally," which is the WRONG question for a bill
  // purchase: bills-catalog-service.js already priced and locked in a
  // SPECIFIC provider (bill.gateway_code) at request time, and the
  // purchase must go to that exact provider, not wherever generic
  // capability routing currently points. With only one bills provider
  // this was invisible; with VTpass added alongside Flutterwave it
  // would silently execute purchases through the wrong provider. Kept
  // here only for source compatibility in case something external
  // still references them — do not wire a new caller to these for
  // catalog-based bill purchases; use getProviderByCode(gateway_code)
  // instead, same as bills-worker.js and getBillStatus/listBillerItems
  // below already do.
