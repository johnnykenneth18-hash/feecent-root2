...
async function processCreateVirtualAccountJob(job) {
  const { account_id, user_id, email, bvn, nin, first_name, last_name, phone } =
    job.payload;
  ...
  //
  // CAPABILITY OVERRIDE: ServiceRegistry's priority/manual-override logic
  // has no concept of "this specific job can only be fulfilled by one
  // provider" — it just picks whoever is priority 1 for virtual_account.
  // Paystack has no NIN path at all (confirmed against Paystack's own
  // Validate Customer docs — BVN only, always), so a NIN-only job handed
  // to Paystack is a guaranteed, non-retryable failure regardless of what
  // the registry says. If the job has no BVN, Flutterwave is used
  // directly, bypassing the registry for this one job only. BVN-present
  // jobs are unaffected and keep going through normal registry routing
  // (Paystack or Flutterwave, whichever is priority 1 today).
  let implementation, providerCode;
  if (!hasBvn && hasNin) {
    implementation = flutterwaveProvider;
    providerCode = "flutterwave";
    console.log(
      `[VA-WORKER] Job ${job.id} has NIN but no BVN — routing directly to Flutterwave ` +
        `(Paystack has no NIN-based verification path), bypassing normal registry priority.`,
    );
  } else {
    try {
      ({ implementation, providerCode } =
        await ServiceRegistry.resolve("virtual_account"));
    } catch (err) {
      await supabase
        .from("accounts")
