...
async function purchaseData({ customerIdentifier, amount, reference, planCode }) {
  if (!planCode) {
    return { success: false, error: "planCode (Flutterwave's bill-category item name) is required for data purchases", retryable: false };
  }
  return purchaseBill({ type: planCode, customerIdentifier, amount, reference });
}
...
async function payCable({ customerIdentifier, amount, reference, planCode }) {
  if (!planCode) {
