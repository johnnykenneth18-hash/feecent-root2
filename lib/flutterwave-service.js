 ...
 * Requires EITHER a BVN or a NIN — Flutterwave's create-virtual-account
 * endpoint accepts either identifier for static/permanent accounts (their
 * NGN Virtual Accounts docs list "BVN or NIN" explicitly; the bulk
 * endpoint's `bulk_data` entries confirm the same field name, `nin`).
 * Unlike Paystack, Flutterwave does NOT require BVN specifically — this
 * is the provider that gets picked when a signup only has a NIN on file.
 *
 * If both are present, BVN is sent (no functional difference to
 * Flutterwave, just keeps the existing behavior for the common case
 * unchanged).
 *
 * @param {Object} params
 * @param {string} params.email
 * @param {string} [params.bvn] - 11-digit BVN
 * @param {string} [params.nin] - 11-digit NIN, used only if bvn is absent
...
async function createVirtualAccount({
  ...
  const hasBvn = bvn && /^\d{11}$/.test(bvn);
  const hasNin = nin && /^\d{11}$/.test(nin);

  if (!hasBvn && !hasNin) {
    return { success: false, error: "Invalid or missing BVN/NIN — one of the two is required" };
  }
  ...
  // Flutterwave takes one identity field at a time — prefer BVN when both
  // are on file since that's the identifier this integration has always
  // used; fall back to NIN when it's the only one available.
  if (hasBvn) body.bvn = bvn;
  else body.nin = nin;
