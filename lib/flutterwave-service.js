...
// type value is my best-known convention for Flutterwave's data bundle
// product — CONFIRM against your account's /bill-categories response
// before relying on this. itemCode should be the plan's
// external_plan_code from bill_plans (the specific bundle SKU, e.g.
// "1GB-30-MTN") — Flutterwave needs this to know which bundle you mean;
// amount alone isn't enough for it to disambiguate between bundles.
...
// type value is my best-known convention for electricity/utility bills
// — CONFIRM against your account. Electricity meters are usually
// PREPAID or POSTPAID; if Flutterwave's schema needs that distinction
// explicitly, it'll need to be threaded through from bill_providers'
// external_metadata (not yet wired — flagging rather than guessing a
// default that could be wrong for a postpaid meter).
...
// type value is my best-known convention for cable/TV subscriptions —
// CONFIRM against your account. itemCode should be the specific
// package's external_plan_code (e.g. a DSTV Compact bundle code).
...
// type value is my best-known convention for betting wallet funding —
