...
// own.
//
// CAPABILITIES declares which service codes (see 011_service_registry.sql)
// this provider is meant to support — used to seed provider_capabilities
// on deploy and by service-registry-admin-service.js's
// assertCapability() check, so the admin panel can't route a service to
// Flutterwave that it doesn't actually implement here.
...
class FlutterwaveProvider extends PaymentProvider {
  ...
  // Confirmed real endpoint (see flutterwave-service.js's
  // validateCustomer() header) — unlike every other bill-payment
  // method above, this one isn't a best-known guess. itemCode is
  // required; bills-service.js's handleVerifyCustomer() is
  // responsible for resolving it (plan's external_item_code, or
  // falling back to the provider row's) before calling this, and for
  // treating a missing itemCode as "skip verification," not calling
  // this without one.
  async verifyCustomer({ customerIdentifier, itemCode }) {
    const result = await flutterwaveService.validateCustomer({
      itemCode,
      customerIdentifier,
    });
    if (!result.success) {
